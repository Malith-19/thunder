...
export default function ApplicationCreatePage(): JSX.Element {
  ...
  const allowEmbeddedApproach =
    selectedTemplateConfig?.type === 'fullstack' || selectedTemplateConfig?.type === 'mobile';
  ...
  const handleCreateApplication = (skipOAuthConfig = false, overrideFlowId?: string): void => {
    ...
    // The canonical application type comes from the selected template. The MCP client template can
    // resolve to two types, so the machine-to-machine selection overrides it.
    const applicationType: ApplicationType | undefined =
      isMcpClientTemplate && mcpClientType === McpClientTypes.M2M ? 'm2m' : selectedTemplateConfig?.type;
      ...
      case ApplicationCreateFlowStep.EXPERIENCE:
        return (
          <ConfigureExperience
            ...
            embeddedRequiresAttestation={selectedTemplateConfig?.type === 'mobile'}
