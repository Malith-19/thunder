	...
	*/
	static async verifySessionToken(token) {
		try {
			...
			throw new ThunderIDRuntimeError(`Invalid session token: ${error$1 instanceof Error ? error$1.message : "Unknown error"}`, "invalid-session-token", "nextjs", "Session token verification failed");
		}
	}
	/**
	...
	*/
	static async verifySessionTokenForRefresh(token) {
		try {
			...
			throw new ThunderIDRuntimeError(`Invalid session token: ${error$1 instanceof Error ? error$1.message : "Unknown error"}`, "invalid-session-token-for-refresh", "nextjs", "Session token signature or type check failed during refresh");
		}
	}
	/**
	...
	*/
	static async verifyTempSession(token) {
		try {
			...
			throw new ThunderIDRuntimeError(`Invalid temporary session token: ${error$1 instanceof Error ? error$1.message : "Unknown error"}`, "invalid-temp-session-token", "nextjs", "Temporary session token verification failed");
		}
	}
	/**
...
//#endregion
export { createRouteMatcher_default as createRouteMatcher, thunderIDMiddleware_default as thunderIDMiddleware };