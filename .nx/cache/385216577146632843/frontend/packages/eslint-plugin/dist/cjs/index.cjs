...
__vitest_eslint_plugin = __toESM(__vitest_eslint_plugin);
...
const javascriptConfig = [
	...
	eslint_plugin_import_x.importX.flatConfigs.recommended,
	{
...
const playwrightConfig = [{
	...eslint_plugin_playwright.default.configs["flat/recommended"],
	files: [
		"**/tests/**/*.ts",
		"**/*.spec.ts",
		"**/*.spec.js"
...
function hasTsconfigFile(directory, configFiles) {
	return configFiles.some((configFile) => {
		return (0, fs.existsSync)((0, path.join)(directory, configFile));
	});
}
/**
...
function resolveTsconfigRootDir(startDir = process.cwd()) {
	let currentDir = (0, path.resolve)(startDir);
	const rootDir = (0, path.parse)(currentDir).root;
...
function createParserOptions(options = []) {
	const { additionalPatterns = [], tsconfigRootDir: manualTsconfigRootDir, project, maximumDefaultProjectFileMatchCount = 20 } = Array.isArray(options) ? { additionalPatterns: options } : options;
	...
	const tsconfigRootDir = manualTsconfigRootDir ? (0, path.resolve)(manualTsconfigRootDir) : resolveTsconfigRootDir();
...
const reactConfig = [
	...
	} },
	{
		...
		settings: {
			...
			"import-x/resolver-next": [(0, eslint_import_resolver_typescript.createTypeScriptImportResolver)({ alwaysTryTypes: true })]
		}
	},
	{
...
const typescriptConfig = [
	...
	} },
	{
		...
		...typescript_eslint.default.configs.disableTypeChecked
	},
	{
		name: "thunderid/typescript-resolver",
		settings: { "import-x/resolver-next": [(0, eslint_import_resolver_typescript.createTypeScriptImportResolver)({ alwaysTryTypes: true })] }
	},
	{
...
const vitestConfig = [{
	...__vitest_eslint_plugin.default.configs.recommended,
	files: [
		"**/*.test.ts",
		"**/*.test.tsx",
		"**/*.spec.ts",
	...
	rules: {
		...__vitest_eslint_plugin.default.configs.recommended.rules,
...
const vueConfig = [
	...eslint_plugin_vue.default.configs["flat/recommended"],
	{ languageOptions: {
		ecmaVersion: 2020,
		globals: globals.default.browser,
		parserOptions: {
...
//#endregion
//#region src/index.ts
const pkgPath = (0, path.join)((0, path.dirname)((0, url.fileURLToPath)(require("url").pathToFileURL(__filename).href)), "../package.json");
