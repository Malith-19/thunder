...
function useTemplateLiteralResolver() {
	return {
		resolve: (0, react.useMemo)(() => (value, handlers) => {
			if (!value || typeof value !== "string") return;
			const match = __thunderid_utils.TEMPLATE_LITERAL_REGEX.exec(value);
			if (!match) return value;
			const parsed = (0, __thunderid_utils.parseTemplateLiteral)(match[1].trim());
			if (parsed.key && handlers?.[parsed.type]) return handlers[parsed.type](parsed.key);
			return parsed.key ?? value;
		}, []),
		resolveAll: (0, react.useMemo)(() => (value, handlers) => {
			if (!value || typeof value !== "string") return;
			return value.replace(/\{\{\s*([^}]+)\s*\}\}/g, (fullMatch, inner) => {
				const parsed = (0, __thunderid_utils.parseTemplateLiteral)(inner.trim());
				if (parsed.key && handlers?.[parsed.type]) return handlers[parsed.type](parsed.key);
				return fullMatch;
			});
		}, [])
...
function useResolveDisplayName({ handlers }) {
	const { resolve } = useTemplateLiteralResolver();
	return { resolveDisplayName: (0, react.useCallback)((displayName) => {
		if (!displayName.trim()) return "";
		if ((0, __thunderid_utils.isI18nTemplatePattern)(displayName)) {
...
function useCopyToClipboard(options = {}) {
	...
	*/
	...
	return {
		copied,
		copy: (0, react.useCallback)(async (text) => {
			try {
...
const logger = (0, __thunderid_logger.createLogger)({ level: "warn" });
/**
...
exports.useDataGridLocaleText = useDataGridLocaleText;
exports.useResolveDisplayName = useResolveDisplayName;
exports.useTemplateLiteralResolver = useTemplateLiteralResolver;