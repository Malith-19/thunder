...
//#endregion
let __tanstack_react_router = require("@tanstack/react-router");
__tanstack_react_router = __toESM(__tanstack_react_router);
...
* ```
*/
const ProtectedRoute = ({ children, fallback, redirectTo, loader = null }) => {
	const { isSignedIn, isLoading } = (0, __thunderid_react.useThunderID)();
	if (isLoading) return loader;
	if (isSignedIn) return children;
	if (fallback) return fallback;
	if (redirectTo) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(__tanstack_react_router.Navigate, { to: redirectTo });
	throw new __thunderid_react.ThunderIDRuntimeError("\"fallback\" or \"redirectTo\" prop is required.", "ProtectedRoute-ValidationError-001", "tanstack-router", "Either \"fallback\" or \"redirectTo\" prop must be provided to handle unauthenticated users.");
};
...
const CallbackRoute = ({ onError, onNavigate }) => {
	const router = (0, __tanstack_react_router.useRouter)();
	const { pathname } = (0, __tanstack_react_router.useRouterState)().location;
	const handleNavigate = (path) => {
		...
		const basename = fullPath.endsWith(pathname) ? fullPath.slice(0, -pathname.length).replace(/\/$/, "") : "";
		const navigationPath = basename && path.startsWith(basename) ? path.slice(basename.length) || "/" : path;
		router.navigate({ to: navigationPath }).catch(() => {});
	};
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(__thunderid_react.Callback, {
