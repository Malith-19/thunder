const require_getMappedUserProfileValue = require('./getMappedUserProfileValue.cjs');
...
const getDisplayName = (mergedMappings, user, displayAttributes) => {
	...
	if (displayAttributes && displayAttributes.length > 0) {
		...
		displayAttributes.some((attr) => {
			const value = require_getMappedUserProfileValue.default(attr, mappings, user);
	...
	const firstName = require_getMappedUserProfileValue.default("firstName", mappings, user);
	const lastName = require_getMappedUserProfileValue.default("lastName", mappings, user);
	if (firstName && lastName) return `${firstName} ${lastName}`;
	return require_getMappedUserProfileValue.default("username", mappings, user) || require_getMappedUserProfileValue.default("email", mappings, user) || require_getMappedUserProfileValue.default("name", mappings, user) || "User";
};
var getDisplayName_default = getDisplayName;

//#endregion
exports.default = getDisplayName_default;
