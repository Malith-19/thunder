...
const BaseSignUpContent = ({ afterSignUpUrl, onInitialize, onSubmit, onError, onFlowChange, onComplete, error: externalError, className = "", inputClassName = "", buttonClassName = "", errorClassName = "", messageClassName = "", size = "medium", variant = "outlined", isInitialized, children, showTitle = true, showSubtitle = true }) => {
	...
	*/
	const isDisplayOnlyCompletion = (response) => {
		...
		return response?.flowStatus === __thunderid_browser.EmbeddedSignUpFlowStatusV2.Complete && Array.isArray(components) && components.length > 0 && !response?.assertion && !data?.redirectURL && !response?.redirectUrl;
	};
	/**
	...
	*/
	const handleRedirectionIfNeeded = (response) => {
		if (response?.type === __thunderid_browser.EmbeddedSignUpFlowTypeV2.Redirection && (response?.data)?.redirectURL) {
			/**
				...
				*/
				...
				if (code && state) {
					...
					try {
						...
						if (continueResponse.flowStatus === __thunderid_browser.EmbeddedSignUpFlowStatusV2.Error) {
							handleError(continueResponse);
							onError?.(continueResponse);
						} else if (continueResponse.flowStatus === __thunderid_browser.EmbeddedSignUpFlowStatusV2.Complete) handleFlowCompletion(continueResponse);
						else if (continueResponse.flowStatus === __thunderid_browser.EmbeddedSignUpFlowStatusV2.Incomplete) {
	...
	*/
	const handleSubmit = async (component, data, skipValidation) => {
		...
		try {
			...
			if (response.flowStatus === __thunderid_browser.EmbeddedSignUpFlowStatusV2.Error) {
				handleError(response);
				onError?.(new Error(require_flowTransformer.extractErrorMessage(response, t)));
				return;
			}
			if (response.flowStatus === __thunderid_browser.EmbeddedSignUpFlowStatusV2.Complete) {
				handleFlowCompletion(response);
				return;
			}
			if (response.flowStatus === __thunderid_browser.EmbeddedSignUpFlowStatusV2.Incomplete) {
	...
	*/
	(0, react.useEffect)(() => {
		...
		const performPasskeyRegistration = async () => {
			...
			if (processedResponse.flowStatus === __thunderid_browser.EmbeddedSignUpFlowStatusV2.Complete) handleFlowCompletion(processedResponse);
			else {
	...
	*/
	const renderComponents = (0, react.useCallback)((components) => require_AuthOptionFactory.renderSignUpComponents(components, formValues, touchedFields, formErrors, isLoading, isFormValid, handleInputChange, {
	...
	*/
	...
	const { title, subtitle, componentsWithoutHeadings } = require_getAuthComponentHeadings.default(currentFlow.data?.components || [], flowTitle, flowSubtitle, t("signup.heading"), t("signup.subheading"));
