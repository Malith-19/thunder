...
const require_passkey = require('../../../../../utils/v2/passkey.cjs');
...
const SignIn = ({ className, preferences, size = "medium", onSuccess, onError, variant, children }) => {
	const { applicationId, afterSignInUrl, signIn, isInitialized, isLoading, meta, getStorageManager, scopes } = require_useThunderID.default();
	...
	const [serverFieldErrors, setServerFieldErrors] = (0, react.useState)(null);
	...
	*/
	const clearFlowState = async () => {
		...
		try {
			...
			__thunderid_browser.logger.warn("Failed to clear authId from hybrid storage.");
		}
	...
	*/
	const handleAuthId = async (authId) => {
		if (authId) try {
			...
			__thunderid_browser.logger.warn("Failed to store authId in hybrid storage.");
		}
	};
	/**
	...
	const initializeFlow = async () => {
		...
		try {
			...
			if (resumeExecutionId) try {
				...
				if (storedExecutionId && resumeExecutionId === storedExecutionId && resumeError instanceof __thunderid_browser.ThunderIDAPIError && resumeError.statusCode === 400) {
					setExecutionId(null);
					try {
						await (await getStorageManager())?.removeHybridDataParameter?.("authId");
					} catch {
					...
					if (!effectiveApplicationId) {
						setError(new __thunderid_browser.ThunderIDRuntimeError(t("errors.signin.session.expired") || "Your session has expired. Please return to the application and sign in again.", "SIGN_IN_ERROR", "react"));
						return;
					}
					response = await signIn({
			...
			const { executionId: normalizedExecutionId, components: normalizedComponents, additionalData: normalizedAdditionalData } = require_flowTransformer.normalizeFlowResponse(response, t, { resolveTranslations: false }, meta);
			await setChallengeToken(response.challengeToken ?? null);
			...
			setError(err instanceof __thunderid_browser.ThunderIDRuntimeError ? err : new Error(require_flowTransformer.extractErrorMessage(err, t)));
	...
	const handleSubmit = async (payload) => {
		...
		try {
			...
			if (response.flowStatus === __thunderid_browser.EmbeddedSignInFlowStatusV2.Error) {
				await clearFlowState();
				const err = new Error(require_flowTransformer.extractErrorMessage(response, t));
				setError(err);
				cleanupFlowUrlParams();
				throw err;
			}
			if (response.flowStatus === __thunderid_browser.EmbeddedSignInFlowStatusV2.Complete) {
				...
				try {
					...
					__thunderid_browser.logger.warn("Failed to clear authId from hybrid storage after completion.");
				}
				cleanupOAuthUrlParams(true);
			...
			if (normalizedExecutionId && normalizedComponents) {
				...
				if (response?.error) setFlowError(new Error(require_flowTransformer.extractErrorMessage(response, t)));
			}
		} catch (error) {
			const err = error;
			await clearFlowState();
	...
	(0, react.useEffect)(() => {
		...
		const performPasskeyProcess = async () => {
			...
			if (passkeyState.challenge) {
				const passkeyResponse = await require_passkey.handlePasskeyAuthentication(passkeyState.challenge);
				...
				const passkeyResponse = await require_passkey.handlePasskeyRegistration(passkeyState.creationOptions);
				const passkeyResponseObj = JSON.parse(passkeyResponse);
				inputs = {
