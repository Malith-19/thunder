...
import { EmbeddedSignInFlowStatusV2, EmbeddedSignUpFlowStatusV2, ThunderIDBrowserClient, ThunderIDRuntimeError, deriveOrganizationHandleFromBaseUrl, executeEmbeddedRecoveryFlowV2, executeEmbeddedSignInFlowV2, executeEmbeddedSignUpFlowV2, extractUserClaimsFromIdToken, isEmpty } from "@thunderid/browser";
...
var ThunderIDReactClient = class extends ThunderIDBrowserClient {
	...
	reInitialize(config) {
		return this.withLoading(async () => {
			...
			try {
				...
				throw new ThunderIDRuntimeError(`Failed to check if the client is initialized: ${error instanceof Error ? error.message : String(error)}`, "ThunderIDReactClient-reInitialize-RuntimeError-001", "react", "An error occurred while checking the initialization status of the client.");
			}
	...
	async getMyOrganizations(options) {
		try {
			...
			throw new ThunderIDRuntimeError(`Failed to fetch the user's associated organizations: ${error instanceof Error ? error.message : String(error)}`, "ThunderIDReactClient-getMyOrganizations-RuntimeError-001", "react", "An error occurred while fetching associated organizations of the signed-in user.");
		}
	}
	async getAllOrganizations(options) {
		try {
			...
			throw new ThunderIDRuntimeError(`Failed to fetch all organizations: ${error instanceof Error ? error.message : String(error)}`, "ThunderIDReactClient-getAllOrganizations-RuntimeError-001", "react", "An error occurred while fetching all the organizations associated with the user.");
		}
	}
	async getCurrentOrganization() {
		try {
			...
			throw new ThunderIDRuntimeError(`Failed to fetch the current organization: ${error instanceof Error ? error.message : String(error)}`, "ThunderIDReactClient-getCurrentOrganization-RuntimeError-001", "react", "An error occurred while fetching the current organization of the signed-in user.");
		}
	}
	async switchOrganization(organization) {
		return this.withLoading(async () => {
			try {
				...
				if (!organization.id) throw new ThunderIDRuntimeError("Organization ID is required for switching organizations", "react-ThunderIDReactClient-SwitchOrganizationError-001", "react", "The organization object must contain a valid ID to perform the organization switch.");
				...
				throw new ThunderIDRuntimeError(`Failed to switch organization: ${error.message || error}`, "react-ThunderIDReactClient-SwitchOrganizationError-003", "react", "An error occurred while switching to the specified organization. Please try again.");
			}
		});
	}
	isLoading() {
	...
	async signUp(...args) {
		...
		const response = await executeEmbeddedSignUpFlowV2({
			authId,
			baseUrl,
		...
		if (response && typeof response === "object" && response.flowStatus === EmbeddedSignUpFlowStatusV2.Complete && response.assertion) {
			const decodedAssertion = await this.decodeJwtToken(response.assertion);
			const createdAt = decodedAssertion.iat ? decodedAssertion.iat * 1e3 : Date.now();
			const expiresIn = decodedAssertion.exp && decodedAssertion.iat ? decodedAssertion.exp - decodedAssertion.iat : 3600;
			await this.setSession({
...
//#endregion
export { ThunderIDReactClient_default as default };
//# sourceMappingURL=ThunderIDReactClient.js.map