...
function renderTemplateFile(templatePath, context) {
	...
	return renderTemplate(readFileSync(templatePath, "utf8"), context);
}
...
function createFileFromTemplate(templatePath, outputPath, context) {
	...
	try {
		chmodSync(outputPath, statSync(templatePath).mode & 511);
	} catch (err) {
...
function getWorkspaceInfo() {
	...
	while (currentDir !== fsRoot && currentDir !== ".") {
		...
		if (existsSync(frontendDir) && existsSync(frontendNxJson) && existsSync(frontendPackageJson)) try {
			const parsed = JSON.parse(readFileSync(frontendPackageJson, "utf8"));
		...
		if (existsSync(packageJsonPath) && existsSync(nxJsonPath)) try {
...
async function createFeature() {
	...
	try {
		...
		[
			...
			"src/pages",
			"src/utils"
		].forEach((dir) => ensureDir(join(featureDir, dir)));
		const copyTemplateFiles = (templateDir, targetDir, ctx) => {
			readdirSync(templateDir, { withFileTypes: true }).forEach((entry) => {
				const templatePath = join(templateDir, entry.name);
				const targetPath = join(targetDir, renderTemplate(entry.name, ctx));
				if (entry.isDirectory()) {
					ensureDir(targetPath);
					copyTemplateFiles(templatePath, targetPath, ctx);
				} else if (entry.isFile() && entry.name.endsWith(".hbs")) createFileFromTemplate(templatePath, join(targetDir, renderTemplate(entry.name.replace(/\.hbs$/, ""), ctx)), ctx);
			});
		};
		copyTemplateFiles(join(getTemplateDir(), "feature", "src"), join(featureDir, "src"), context);
		createFileFromTemplate(join(getTemplateDir(), "feature", "package.json.hbs"), join(featureDir, "package.json"), context);
		[
			"tsconfig.json",
			"tsconfig.lib.json",
			"tsconfig.spec.json",
			...
			".gitignore",
			".prettierignore"
		].forEach((file) => createFileFromTemplate(join(getTemplateDir(), "feature", `${file}.hbs`), join(featureDir, file), context));
...
async function createPackage() {
	...
	try {
		...
		createFileFromTemplate(join(getTemplateDir(), "package", `${templateBase}`, "package.json.hbs"), join(packageDir, "package.json"), context);
		[
			"tsconfig.json",
			"tsconfig.lib.json",
			"tsconfig.spec.json",
			...
			".gitignore",
			".prettierignore"
		].forEach((file) => createFileFromTemplate(join(getTemplateDir(), "package", `${templateBase}`, `${file}.hbs`), join(packageDir, file), context));
		createFileFromTemplate(join(getTemplateDir(), "package", `${templateBase}`, "src", "index.ts.hbs"), join(packageDir, "src", "index.ts"), context);
		createFileFromTemplate(join(getTemplateDir(), "package", `${templateBase}`, "README.md.hbs"), join(packageDir, "README.md"), context);
...
async function main() {
	...
	intro("\n" + [
		...
		colors.cyanBright("   ╚═╝   ╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═══╝╚═════╝ ╚══════╝╚═╝  ╚═╝") + colors.magentaBright("╚═╝╚═════╝")
          ...
          ${colors.yellow("⚡")} ${colors.bold(colors.white("ThunderID"))}${colors.dim(colors.gray(" · Frontend Scaffolding Tool"))}\n`);
	const workspaceInfo = getWorkspaceInfo();
...
//#endregion
export { registerHandlebarsHelpers as a, ensureDir as c, renderTemplate as d, validateName as i, createFileFromTemplate as l, createPackage_default as n, getWorkspaceInfo as o, createFeature_default as r, getTemplateDir as s, cli_default as t, renderTemplateFile as u };