...
var ThunderIDExpressClient = class extends __thunderid_node.ThunderIDNodeClient {
	_expressConfig;
	constructor() {
		super();
	}
	...
	async signIn(req, res, next, signInConfig) {
		if (expressUtils_default(req.originalUrl)) return Promise.reject(new __thunderid_node.ThunderIDAuthException("EXPRESS-CLIENT-SI-IV01", "Invalid login request URL", "Login request contains an error query parameter in the URL"));
		let userId = req.cookies?.[SESSION_COOKIE_NAME];
		if (!userId) userId = (0, uuid.v4)();
...
const handleSignIn = () => {
	return async (req, res, next) => {
		...
		if (!client) {
			__thunderid_node.logger.error("thunderID() middleware must be mounted before handleSignIn()");
			res.status(500).end();
			return;
		}
		...
		const onError = config?.onError ?? ((r, e) => {
			__thunderid_node.logger.error(e.message);
			r.status(500).end();
		});
		try {
...
const handleSignOut = () => {
	return async (req, res) => {
		const client = req.thunderIDAuth;
		if (!client) {
			__thunderid_node.logger.error("thunderID() middleware must be mounted before handleSignOut()");
			res.status(500).end();
			return;
		}
		const config = client.expressConfig;
		...
		const sessionId = req.cookies?.[SESSION_COOKIE_NAME];
		if (!sessionId) {
			onError(res, new __thunderid_node.ThunderIDRuntimeError("No cookie found in the request", "EXPRESS-AUTH_MW-LOGOUT-NF01", "express"));
			return;
		}
		try {
...
const protect = (onUnauthenticated) => {
	return async (req, res, next) => {
		...
		if (!client || !sessionId) {
			__thunderid_node.logger.error("No session ID found in the request cookies");
			reject();
			return;
		}
		if (await client.isSignedIn(sessionId) ?? false) return next();
		__thunderid_node.logger.error("Invalid session ID found in the request cookies");
		reject();
	};
};
...
const handleFlow = () => {
	return async (req, res) => {
		const client = req.thunderIDAuth;
		if (!client) {
			__thunderid_node.logger.error("thunderID() middleware must be mounted before handleFlow()");
		...
		try {
			...
			const flowResponse = await (0, __thunderid_node.executeEmbeddedSignInFlowV2)({
...
exports.ThunderIDExpressClient = ThunderIDExpressClient_default;
exports.handleFlow = flow_default;
exports.handleSignIn = handleSignIn;
exports.handleSignOut = handleSignOut;
exports.protect = protect_default;
exports.thunderID = thunderID;
Object.keys(__thunderid_node).forEach(function (k) {
  if (k !== 'default' && !Object.prototype.hasOwnProperty.call(exports, k)) Object.defineProperty(exports, k, {
    enumerable: true,
    get: function () { return __thunderid_node[k]; }
  });
});
