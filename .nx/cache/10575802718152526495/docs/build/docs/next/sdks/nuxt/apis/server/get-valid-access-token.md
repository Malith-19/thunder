...
- To access session metadata alongside the token, call [`requireServerSession()`](https://thunderid.dev/docs/next/sdks/nuxt/apis/server/require-server-session.md) first and use `getValidAccessToken` only when you need to forward the token to an external API.
