...
A minimal setup involves wrapping your root component with `` and providing the required `client-id` and `base-url` props.
...
| `signInUrl` | `string` | ❌ | Custom sign-in URL. When set, `` navigates to this URL instead of triggering the embedded flow. |
| `signUpUrl` | `string` | ❌ | Custom sign-up URL. When set, `` navigates to this URL instead of triggering the embedded flow. |
