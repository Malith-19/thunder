...
|-----------|------|-------------|
| `config` | `AuthClientConfig` | Application configuration. See [Configuration Reference](https://thunderid.dev/docs/next/sdks/javascript/apis/configuration.md) |
...
```ts
const claims = await client.decodeJwtToken<>(token)
...
| `config.data` | `Record<string, string>` | POST body fields. Use `}` to substitute the current access token |
...
```ts
const url = await client.getOBOSignInURL()
...
```ts
const  = await client.initiateCIBA({
...
| `options.loginHintToken` | `string` | — | Defined in the CIBA specification but **not supported** by ThunderID. The server returns `invalid_request` when this parameter is provided. |
