...
The TanStack Router `ProtectedRoute` does not include `onSignIn` or `signInOptions` props. For custom sign-in logic, use the [`useThunderID()`](https://thunderid.dev/docs/next/sdks/react/apis/hooks/use-thunderid.md) hook directly in the protected component or layout.
