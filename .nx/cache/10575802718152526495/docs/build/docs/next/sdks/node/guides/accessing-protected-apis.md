...
- Complete the [Handling Authentication](https://thunderid.dev/docs/next/sdks/node/guides/handling-authentication.md) guide to set up the client and session management.
...
export async function fetchDashboardData(sessionId: string) {
  ...
  const [profile, organizations] = await Promise.all([
    fetch('https://localhost:8090/users/<user_id>', ).then(r => r.json()),
    fetch('https://localhost:8090/organizations', ).then(r => r.json()),
  ])
...
- [`ThunderIDNodeClient`](https://thunderid.dev/docs/next/sdks/node/apis/clients/thunderid-node-client.md) — Base client class reference
