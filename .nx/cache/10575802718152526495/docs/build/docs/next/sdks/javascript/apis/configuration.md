...
By default the SDK fetches `/.well-known/openid-configuration` to resolve endpoint URLs.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `discovery.wellKnown.enabled` | `boolean` | `true` | Whether to use the well-known discovery document to resolve endpoints |
| `endpoints.wellKnown` | `string` | `/.well-known/openid-configuration` | Override the discovery document URL |
