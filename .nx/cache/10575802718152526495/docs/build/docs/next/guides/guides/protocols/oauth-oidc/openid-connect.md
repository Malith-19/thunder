...
**OpenID Connect Core 1.0** ([OIDC Core](https://openid.net/specs/openid-connect-core-1_0.html)) is the identity layer on top of OAuth 2.1. Where OAuth tells you *what* a client can do, OIDC tells you *who* the user is. ThunderID implements OIDC Core as a thin layer over its OAuth 2.1 authorization code grant — adding the `openid` scope, the ID Token, the [UserInfo](https://thunderid.dev/docs/next/guides/guides/protocols/userinfo.md) endpoint, and a small set of authentication-context request parameters.
...
| ID Token format | JWS by default; can be configured per app as JWE or NESTED_JWT (see [Token Formats](https://thunderid.dev/docs/next/guides/guides/protocols/token-formats.md)) |
| UserInfo | `GET /oauth2/userinfo` — see [UserInfo](https://thunderid.dev/docs/next/guides/guides/protocols/userinfo.md) |
| Standard scopes | `openid`, `profile`, `email`, `phone`, `address` — see [Claims & Scopes](https://thunderid.dev/docs/next/guides/guides/protocols/claims-and-scopes.md) |
| Subject types | `public` only — pairwise subject identifiers are not supported |
| Discovery | `/.well-known/openid-configuration` — see [Server Metadata](https://thunderid.dev/docs/next/guides/guides/protocols/server-metadata.md) |
...
Additional user claims (e.g. `name`, `email`) are added based on the requested scopes — see [Claims & Scopes](https://thunderid.dev/docs/next/guides/guides/protocols/claims-and-scopes.md).
...
| `claims` | Request specific claims claim-by-claim | See [Claims & Scopes](https://thunderid.dev/docs/next/guides/guides/protocols/claims-and-scopes.md) |
...
3. Optionally configure ID Token format, signing algorithm, and lifetime on the same tab (see [Token Formats](https://thunderid.dev/docs/next/guides/guides/protocols/token-formats.md)).
4. Save.
...
- [UserInfo](https://thunderid.dev/docs/next/guides/guides/protocols/userinfo.md) — the endpoint that returns claims about the authenticated user
- [Claims & Scopes](https://thunderid.dev/docs/next/guides/guides/protocols/claims-and-scopes.md) — how scope requests map to claims in the ID Token and UserInfo
- [Token Formats](https://thunderid.dev/docs/next/guides/guides/protocols/token-formats.md) — JWS, JWE, and NESTED_JWT options for the ID Token
- [Authorization Code](https://thunderid.dev/docs/next/guides/guides/protocols/authorization-code.md) — the underlying OAuth flow
- [Server Metadata](https://thunderid.dev/docs/next/guides/guides/protocols/server-metadata.md) — the `/.well-known/openid-configuration` document
