...
- An organization unit to associate the resource server with. See [Organization Units](https://thunderid.dev/docs/next/guides/organization-units.md) to create one.
...
Permission strings use one of two forms. Flat permissions follow `` (for example, `booking-api:list`). Hierarchical permissions follow `` (for example, `booking-api:reservations:create`). The resource server handle always prefixes the permission string.

- **Resource Server** — container for all resources and actions. Requires a unique `handle` and an optional `identifier`. When the identifier is set to an absolute URI, the resource server is used in [resource indicator](https://thunderid.dev/docs/next/guides/resource-indicators.md) flows (RFC 8707).
...
- **Use absolute URIs as identifiers for RFC 8707.** The `identifier` field accepts any string, but only resource servers with absolute URI identifiers (such as `https://api.example.com/booking`) are used in [resource indicator](https://thunderid.dev/docs/next/guides/resource-indicators.md) flows. Set a URI identifier if you need audience-restricted access tokens.
...
- [Resource Indicators](https://thunderid.dev/docs/next/guides/resource-indicators.md) — request audience-restricted access tokens targeting specific resource servers.
- [Organization Units](https://thunderid.dev/docs/next/guides/organization-units.md) — organize resource servers within organizational boundaries.
