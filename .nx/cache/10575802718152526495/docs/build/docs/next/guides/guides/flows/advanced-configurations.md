...
**Prerequisites:**
- An SMS sender must be configured in ThunderID. See [SMS Providers](https://thunderid.dev/docs/next/guides/notifications/sms-providers.md).
...
- An email service must be configured in ThunderID. See [SMTP Server Configuration](https://thunderid.dev/docs/next/guides/smtp-server/smtp-server-configuration.md). The executor returns failure status if no email client is configured.
...
**Prerequisites:** None. Any context data referenced via `}` placeholders in the URL, headers, or body must be available from prior executors at runtime.
...
2. Resolves `}` placeholders in the URL, headers, and body using runtime data and user inputs from prior executors
...
| Property | UI Label | Required | Default | Description |
|---|---|---|---|---|
| `url` | URL | Yes | — | The endpoint URL (supports placeholders like `}`) |
...
**Placeholder syntax:** Use `}` to reference data from the flow context. Placeholders are resolved from runtime data first, then user inputs. If a placeholder cannot be resolved, it is kept as-is in the request.

The following fields are always available when the corresponding data exists in the flow context:

| Placeholder | Source | Description |
|---|---|---|
| `}` | Authenticated user or runtime data | The authenticated user's ID. Available after any authentication or provisioning executor has run. |
| `}` | Authenticated user or runtime data | The organizational unit ID of the current user. Available after OU resolution or authentication. |
| `}` | OU service lookup | The handle of the resolved OU. Fetched automatically when referenced. |
| `}` | OU service lookup | The display name of the resolved OU. Fetched automatically when referenced. |
| `}` | OU service lookup | The description of the resolved OU. Fetched automatically when referenced. |
| `}` | OAuth flow context | The OAuth client ID of the application initiating the flow. Available in OAuth/OIDC authorization flows. |

In addition to these fields, any value present in runtime data (set by prior executors) or user inputs (collected from prior Views) can be referenced using the same `}` syntax (e.g., `}`, `}`).
...
**Prerequisites:**
- The OpenID4VP service must be configured in ThunderID with at least one presentation definition (see the [OpenID4VP protocol guide](https://thunderid.dev/docs/next/guides/guides/protocols/openid4vc/openid4vp.md)).
...
- [Build a Flow](https://thunderid.dev/docs/next/guides/guides/build-a-flow.md) — Step-by-step guide to creating a flow in the ThunderID Console.
