...
Both patterns use the token endpoint at `POST /oauth2/token`. The agent must be [registered with an OAuth inbound auth config](https://thunderid.dev/docs/next/guides/guides/manage-agents.md) before it can request tokens.
...
ThunderID intersects the requested scopes against the scopes authorized for the agent via its group memberships and role assignments. Requesting a scope the agent is not authorized for results in `invalid_scope`.
...
ThunderID implements [RFC 8693](https://www.rfc-editor.org/rfc/rfc8693) token exchange.
...
If both the `subject_token`'s audiences and the requested `resource` are specified, ThunderID merges them per RFC 8693 §2.1.
...
| `iss` | ThunderID instance URL. | ThunderID instance URL. |
...
| `iss` | Matches your ThunderID instance URL. |
...
- [Manage Agents](https://thunderid.dev/docs/next/guides/guides/manage-agents.md) — Configure OAuth credentials, assign groups, and rotate secrets.
- [Resource Servers](https://thunderid.dev/docs/next/guides/resource-servers.md) — Register resource servers and define the scopes they accept.
- [Resource Indicators](https://thunderid.dev/docs/next/guides/resource-indicators.md) — Use RFC 8707 `resource` parameters to issue audience-restricted tokens.
