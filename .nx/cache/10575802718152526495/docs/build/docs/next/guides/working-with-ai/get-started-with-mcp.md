...
The MCP server is available at `https://localhost:8090/mcp` (or your configured `PUBLIC_URL`). See the [MCP Server guide](https://thunderid.dev/docs/next/guides/working-with-ai/mcp-server.md) for the full list of available tools.

## Prerequisites

- ThunderID server running (default: `https://localhost:8090`)
- ThunderID certificate trusted by your system — add `backend/cmd/server/config/certs/server.cert` to your system's trusted certificate store (Keychain on macOS, certificate store on Linux/Windows)
...
You should see `thunderid-mcp` listed with the MCP server URL.
...
Open the command palette and run **MCP: List Servers** to see `thunderid-mcp`. Click **Start** next to it, or select it and choose **Start Server**. VS Code will prompt for the access token the first time.
...
- [MCP Server](https://thunderid.dev/docs/next/guides/working-with-ai/mcp-server.md) — Full list of available tools and the DCR-based authorization flow
