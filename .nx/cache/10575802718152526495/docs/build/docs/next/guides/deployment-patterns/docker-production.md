...
This guide covers Docker-specific hardening steps for running ThunderID in production. For configuration-level settings such as TLS certificates, encryption keys, and CORS, see the [Production Deployment Guidelines](https://thunderid.dev/docs/next/guides/production-guidelines.md).
...
- Configure the connection in `deployment.yaml`. See [Configure a Production Database](https://thunderid.dev/docs/next/guides/production-guidelines.md#configure-a-production-database) for the full schema.
...
Running a reverse proxy in front of ThunderID separates TLS termination from the application and gives you centralized certificate management. Use [Nginx](https://nginx.org/) or [Traefik](https://traefik.io/).
...
## Next Steps
  ...
  - [Production Deployment Guidelines](https://thunderid.dev/docs/next/guides/production-guidelines.md) — Apply configuration-level hardening: TLS certificates, encryption keys, CORS allowlist, and Redis caching for multi-replica setups.
