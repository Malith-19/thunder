...
# Deploy ThunderID on OpenChoreo

This guide walks you through deploying ThunderID on OpenChoreo using Helm. It covers prerequisites, installation, database configuration, and the built-in environment management that OpenChoreo provides.
...
Clone the ThunderID repository if you have not already done so. Then install ThunderID using the OpenChoreo Helm chart:
...
ThunderID requires PostgreSQL for the identity, runtime, and user databases.
