...
The protocol uses a polling model with two parallel channels: a **back-channel** between ThunderID and the relying party, and an **out-of-band channel** to the wallet via a QR code or deep link.
...
5. ThunderID decrypts the response, verifies the SD-JWT VC signature and key binding, and enforces the presentation policy.
6. The relying party polls `GET /openid4vp/status/` until ThunderID returns `COMPLETED` with a signed `result_token`.
...
| **Request object (JAR)** | ThunderID signs the authorization request as a compact JWS and serves it at `GET /openid4vp/request?state=...` with `Content-Type: application/oauth-authz-req+jwt`. |
...
| **Issuer trust** | When `enforce_trusted_issuer` is enabled, ThunderID verifies the SD-JWT issuer signature against a pinned certificate for the issuer URL. |
...
| `/openid4vp/status/` | `GET` | Poll for transaction status. Returns `PENDING`, `FAILED`, `EXPIRED`, or `COMPLETED` with a `result_token`. |
...
When the relying party polls the status endpoint and receives `COMPLETED`, the response includes a signed JWT `result_token`. Validate the token against the ThunderID JWKS before trusting its claims.
...
| `trusted_issuers` | No | List of `` entries. Required when `enforce_trusted_issuer: true`. |
...
- A certificate-backed signing key (`signing_key_id`) and an ephemeral encryption key (`ephemeral_key_id`) must be registered in the ThunderID key store.
...
Decode and verify the `result_token` signature against the ThunderID [JWKS](https://thunderid.dev/docs/next/guides/guides/protocols/oauth-oidc/jwks.md) endpoint, then read `verified_claims` to extract the user's identity attributes.
...
See [Advanced Configurations — OpenID4VP Verify](https://thunderid.dev/docs/next/guides/guides/flows/advanced-configurations.md#verifiable-credentials) for the executor reference.

## Related Guides

- [Advanced Configurations](https://thunderid.dev/docs/next/guides/guides/flows/advanced-configurations.md) — Flow executor reference, including the OpenID4VP Verify executor
- [JWKS](https://thunderid.dev/docs/next/guides/guides/protocols/oauth-oidc/jwks.md) — Verify the `result_token` signature against the ThunderID public key endpoint
- [Flows](https://thunderid.dev/docs/next/guides/guides/flows/what-are-flows.md) — Build authentication flows that include credential presentation steps
