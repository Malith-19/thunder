...
Whenever a client calls a protected endpoint at ThunderID — token, introspection, PAR, DCR (when secured) — it must prove which client it is. The `token_endpoint_auth_method` setting on the application picks **how**. The choice is fixed per application and applies uniformly to every protected endpoint.

ThunderID supports four methods, three for confidential clients and one (`none`) for public clients. The relevant specs are [RFC 6749 §2.3](https://datatracker.ietf.org/doc/html/rfc6749#section-2.3), [RFC 6749 §2.1](https://datatracker.ietf.org/doc/html/rfc6749#section-2.1) (public clients), and [RFC 7523](https://datatracker.ietf.org/doc/html/rfc7523) (JWT bearer client authentication).
...
The application must have either a **JWKS** (inline JSON Web Key Set) or a **JWKS_URI** (HTTPS URL ThunderID fetches) configured. See [Application certificate](#application-certificate) below for the rules.
...
A "certificate" on a registered application is the public-key material ThunderID uses to:

- Verify signed JWT assertions for `private_key_jwt` client authentication.
- Encrypt ID tokens when [Token Formats](https://thunderid.dev/docs/next/guides/guides/protocols/token-formats.md) for that application is `JWE` or `NESTED_JWT`.
- Sign or encrypt UserInfo responses when [UserInfo](https://thunderid.dev/docs/next/guides/guides/protocols/userinfo.md) response format requires it.
...
This certificate is **separate** from ThunderID's own signing keys (see [JWKS](https://thunderid.dev/docs/next/guides/guides/protocols/jwks.md)), which is the public-key endpoint resource servers use to verify ThunderID-issued tokens.
...
- [Dynamic Client Registration](https://thunderid.dev/docs/next/guides/guides/protocols/dynamic-client-registration.md) — register a client and pick its auth method programmatically
- [Pushed Authorization Requests](https://thunderid.dev/docs/next/guides/guides/protocols/par.md) — uses the same client authentication method
- [Token Introspection](https://thunderid.dev/docs/next/guides/guides/protocols/token-introspection.md) — uses the same client authentication method
- [Token Formats](https://thunderid.dev/docs/next/guides/guides/protocols/token-formats.md) — the application certificate is also used to encrypt ID tokens and UserInfo responses
