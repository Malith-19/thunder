...
1. The client sends `POST /oauth2/bc-authorize` with a user hint (for example, `login_hint`) and the requested scopes.
...
5. ThunderID records the decision and marks the `auth_req_id` accordingly.
...
7. ThunderID returns an access token and, when `openid` is in the scope, an ID token.
...
- [Server Metadata](https://thunderid.dev/docs/next/guides/guides/protocols/server-metadata.md) — the `backchannel_authentication_endpoint` field in the OIDC discovery document
- [Claims & Scopes](https://thunderid.dev/docs/next/guides/guides/protocols/claims-and-scopes.md) — configure which claims appear in the issued tokens
