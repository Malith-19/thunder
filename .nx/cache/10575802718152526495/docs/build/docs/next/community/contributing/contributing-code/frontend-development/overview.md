...
`@monaco-editor/react` loads Monaco from the CDN until `loader.config()` is called. The host app configures this once in `src/lib/monaco-setup.ts`: it points `self.MonacoEnvironment` at locally bundled Vite `?worker` workers and calls `loader.config()` so the loader uses the bundled `monaco-editor`.
...
ThunderID includes a CLI scaffolding tool (`@thunderid/create`) that generates production-ready feature packages. See the [Scaffolding Tool](https://thunderid.dev/docs/next/community/contributing/contributing-code/frontend-development/scaffolding-tool.md) guide for details.
