...
This guide walks you through configuring [KrakenD API Gateway](https://www.krakend.io/) to protect upstream APIs using JSON Web Tokens (JWTs) issued by ThunderID. KrakenD fetches public keys from the ThunderID JWKS endpoint and validates tokens on every request before forwarding traffic to the upstream service.
...
KrakenD validates JWTs using the [`auth/validator`](https://www.krakend.io/docs/authorization/jwt-validation/) component. KrakenD fetches public keys from the ThunderID JWKS endpoint and uses the keys to verify the signature on incoming tokens.
...
| `jwk_url` | `https://<THUNDERID_HOST>:<THUNDERID_PORT>/oauth2/jwks` | ThunderID JWKS endpoint for fetching public keys |
...
```bash
curl --location 'https://<THUNDERID_HOST>:8090/api/server/v1/resource-servers' \
...
```bash
curl --location 'https://<THUNDERID_HOST>:8090/api/server/v1/resource-servers/<RESOURCE_SERVER_ID>/resources' \
  --header 'Authorization: Bearer '"$MGMT_TOKEN"'' \
  --header 'Content-Type: application/json' \
  --data '{
    "name": "Items",
...
```bash
curl --location 'https://<THUNDERID_HOST>:8090/api/server/v1/resource-servers/<RESOURCE_SERVER_ID>/resources/<RESOURCE_ID>/actions' \
  --header 'Authorization: Bearer '"$MGMT_TOKEN"'' \
  --header 'Content-Type: application/json' \
  --data '{
    "name": "Read Item",
...
```bash
curl --location 'https://<THUNDERID_HOST>:8090/api/server/v1/roles' \
  --header 'Authorization: Bearer '"$MGMT_TOKEN"'' \
  --header 'Content-Type: application/json' \
  --data '{
...
```bash
curl --location 'https://<THUNDERID_HOST>:8090/api/server/v1/applications/<APP_ID>/roles' \
  --header 'Authorization: Bearer '"$MGMT_TOKEN"'' \
  --header 'Content-Type: application/json' \
  --data '{
    "roles": ["<ROLE_ID>"]
