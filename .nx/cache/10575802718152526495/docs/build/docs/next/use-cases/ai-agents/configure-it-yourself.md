...
- [Register the Wayfinder Concierge Agent](#register-the-wayfinder-concierge-agent) — needed for [Acting on Its Own](https://thunderid.dev/docs/next/use-cases/try-it-out/act-on-its-own.md) and [Acting on Behalf of a User](https://thunderid.dev/docs/next/use-cases/try-it-out/act-on-behalf-of-user.md).
- [Build the Agent Authentication Flow](#build-the-agent-authentication-flow) — drives the OBO consent screen used by [Acting on Behalf of a User](https://thunderid.dev/docs/next/use-cases/try-it-out/act-on-behalf-of-user.md).
