...
* is currently used.
*/
var FetchHttpClient = class FetchHttpClient extends __thunderid_javascript.HttpClient {
...
var SPAUtils = class SPAUtils {
	...
	*/
	static setSignOutURL(url, clientId, instanceId) {
		sessionStorage.setItem(`${__thunderid_javascript.OIDCRequestConstants.SignOut.Storage.StorageKeys.SIGN_OUT_URL}-instance_${instanceId}-${clientId}`, url);
	}
	/**
	...
	*/
	static getSignOutUrl(clientId, instanceId) {
		return sessionStorage.getItem(`${__thunderid_javascript.OIDCRequestConstants.SignOut.Storage.StorageKeys.SIGN_OUT_URL}-instance_${instanceId}-${clientId}`) ?? "";
	}
	/**
	...
	*/
	static hasErrorInURL(url = window.location.href) {
		...
		return !!urlObject.searchParams.get(ERROR) && urlObject.searchParams.get(STATE_QUERY) !== __thunderid_javascript.OIDCRequestConstants.Params.SIGN_OUT_SUCCESS;
	}
	/**
...
var AuthenticationHelper = class {
	...
	*/
	async exchangeToken(config, enableRetrievingSignOutURLFromSession) {
		...
		else return Promise.reject(new __thunderid_javascript.ThunderIDAuthException("SPA-MAIN_THREAD_CLIENT-RCG-IV01", "Request to the provided endpoint is prohibited.", "Requests can only be sent to resource servers specified by the `allowedExternalUrls` attribute while initializing the SDK."));
	}
	/**
		...
		return httpClient.request(requestConfig).then((response) => {
			...
			if (error?.response?.status === 401 || !error?.response) {
				...
				try {
					...
					throw new __thunderid_javascript.ThunderIDAuthException("SPA-AUTH_HELPER-HR-SE01", refreshError?.name ?? "Refresh token request failed.", refreshError?.message ?? "An error occurred while trying to refresh the access token.");
				}
		...
		return httpClient?.all && httpClient.all(requests).then((responses) => {
			...
			if (error?.response?.status === 401 || !error?.response) {
				try {
					...
					throw new __thunderid_javascript.ThunderIDAuthException("SPA-AUTH_HELPER-HRA-SE01", refreshError?.name ?? "Refresh token request failed.", refreshError?.message ?? "An error occurred while trying to refresh the access token.");
				}
	...
	*/
	async handleSignIn(shouldStopAuthn, checkSession) {
		...
		if (error) {
			...
			throw new __thunderid_javascript.ThunderIDAuthException("SPA-AUTH_HELPER-SI-SE01", error, errorDescription ?? "");
		}
...
const createSessionManagementHelper = async (signOut, setSessionState) => {
	...
	const receivePromptNoneResponse = async (setSessionStateFn) => {
		...
		const sessionState = new URL(window.location.href).searchParams.get(__thunderid_javascript.OIDCRequestConstants.Params.SESSION_STATE);
...
var SPACryptoUtils = class {
	...
	*/
	hashSha256(data) {
		return import_buffer.Buffer.from((0, fast_sha256.default)(new TextEncoder().encode(data)));
	}
	/**
	...
	*/
	verifyJwt(idToken, jwk, algorithms, clientId, issuer, subject, clockTolerance, validateJwtIssuer) {
		...
		return (0, jose.jwtVerify)(idToken, (0, jose.createLocalJWKSet)({ keys: [jwk] }), jwtVerifyOptions).then(() => {
			return Promise.resolve(true);
		}).catch((error) => {
			return Promise.reject(new __thunderid_javascript.ThunderIDAuthException("SPA-CRYPTO-UTILS-VJ-IV01", error?.reason ?? JSON.stringify(error), `${error?.code} ${error?.claim}`));
		});
	}
};
...
var SPAHelper = class {
	...
	*/
	async refreshAccessTokenAutomatically(refreshAccessToken) {
		...
		if (sessionData?.refresh_token) {
			...
			await this._storageManager.setTemporaryDataParameter(__thunderid_javascript.TokenConstants.Storage.StorageKeys.REFRESH_TOKEN_TIMER, JSON.stringify(timer));
		}
	}
	/**
	...
	*/
	async getRefreshTimeoutTimer() {
		const raw = await this._storageManager.getTemporaryDataParameter(__thunderid_javascript.TokenConstants.Storage.StorageKeys.REFRESH_TOKEN_TIMER);
...
const logger$2 = (0, __thunderid_javascript.createPackageComponentLogger)("@thunderid/browser", "ThunderIDBrowserClient");
...
var ThunderIDBrowserClient = class ThunderIDBrowserClient extends __thunderid_javascript.ThunderIDJavaScriptClient {
	...
	async initialize(config, storage) {
		...
		this._sessionManagementHelper = await SessionManagementHelper_default(async () => this.getSignOutUrl(), (sessionState) => sm.setSessionDataParameter(__thunderid_javascript.OIDCRequestConstants.Params.SESSION_STATE, sessionState));
	...
	async _validateMethod(validateAuthentication = true) {
		if (!await this.isInitialized()) return Promise.reject(new __thunderid_javascript.ThunderIDAuthException("SPA-AUTH_CLIENT-VM-NF01", "The SDK is not initialized.", "The SDK must be initialized first."));
		if (validateAuthentication && !await this.isSignedIn()) return Promise.reject(new __thunderid_javascript.ThunderIDAuthException("SPA-AUTH_CLIENT-VM-IV02", "The user is not authenticated.", "The user must be authenticated first."));
		return true;
	}
	isLoading() {
		return false;
	...
	async _signInInternal(signInConfig, authorizationCode, sessionState, state, tokenRequestConfig) {
		...
		if (config?.responseMode === "form_post" && authorizationCode) {
			...
			resolvedAuthorizationCode = new URL(window.location.href).searchParams.get(__thunderid_javascript.OIDCRequestConstants.Params.AUTHORIZATION_CODE) ?? "";
			resolvedSessionState = new URL(window.location.href).searchParams.get(__thunderid_javascript.OIDCRequestConstants.Params.SESSION_STATE) ?? "";
			resolvedState = new URL(window.location.href).searchParams.get(__thunderid_javascript.OIDCRequestConstants.Params.STATE) ?? "";
		...
		return this.getSignInUrl(signInConfig).then(async (url) => {
			if (this._storage === BrowserStorage_default.BrowserMemory && config.enablePKCE) {
				const pkceKey = (0, __thunderid_javascript.extractPkceStorageKeyFromState)(resolvedState);
			...
			if (signInConfig?.response_mode === "direct") {
				...
				return (0, __thunderid_javascript.initializeEmbeddedSignInFlow)({
	...
	async _exchangeCodeForTokens(authorizationCode, sessionState, state, tokenRequestConfig) {
		...
		if (this._storage === BrowserStorage_default.BrowserMemory && config.enablePKCE && sessionState) {
			const pkceKey = (0, __thunderid_javascript.extractPkceStorageKeyFromState)(sessionState);
	...
	getHttpClient() {
		if (!this._httpClient) throw new __thunderid_javascript.ThunderIDAuthException("SPA-AUTH_CLIENT-GHC-NF02", "The SDK is not initialized.", "Call initialize() before getHttpClient().");
	...
	async exchangeToken(config) {
		...
		if (!config.id) return Promise.reject(new __thunderid_javascript.ThunderIDAuthException("SPA-AUTH_CLIENT-RCG-NF01", "The custom grant request id not found.", "Set the `id` attribute on the token exchange config."));
	...
	async on(hook, callback, id) {
		await this.isInitialized();
		if (!callback || typeof callback !== "function") throw new __thunderid_javascript.ThunderIDAuthException("SPA-AUTH_CLIENT-ON-IV02", "Invalid callback function.", "The provided callback must be a function.");
		switch (hook) {
			case "sign-in":
			...
			default: throw new __thunderid_javascript.ThunderIDAuthException("SPA-AUTH_CLIENT-ON-IV01", "Invalid hook.", `Unknown hook: "${hook}"`);
		}
	}
	async _checkSession() {
	...
	async _shouldStopAuthn() {
		...
		return this._sessionManagementHelper.receivePromptNoneResponse(async (sessionState) => {
			await sm.setSessionDataParameter(__thunderid_javascript.OIDCRequestConstants.Params.SESSION_STATE, sessionState ?? "");
		});
	}
	...
	async _constructSilentSignInUrl(additionalParams = {}) {
		...
		if (this._storage === BrowserStorage_default.BrowserMemory && config.enablePKCE) {
			const state = urlObject.searchParams.get(__thunderid_javascript.OIDCRequestConstants.Params.STATE);
			SPAUtils_default.setPKCE((0, __thunderid_javascript.extractPkceStorageKeyFromState)(state ?? ""), await this.getPKCECode(state ?? ""));
		}
...
const logger$1 = (0, __thunderid_javascript.createPackageComponentLogger)("@thunderid/browser", "WebAuthn");
/**
...
const handleWebAuthnAuthentication = async (challengeData) => {
	if (!window.navigator.credentials?.get) throw new __thunderid_javascript.ThunderIDRuntimeError("WebAuthn is not supported in this browser. Please use a modern browser or try a different authentication method.", "browser-webauthn-not-supported", "browser", "WebAuthn/Passkey authentication requires a browser that supports the Web Authentication API.");
	if (window.location.protocol !== "https:" && window.location.hostname !== "localhost") throw new __thunderid_javascript.ThunderIDRuntimeError("Passkey authentication requires a secure connection (HTTPS). Please ensure you are accessing this site over HTTPS.", "browser-webauthn-insecure-connection", "browser", "WebAuthn authentication requires HTTPS for security reasons, except when running on localhost for development.");
	try {
		...
		const adjustedOptions = {
			...
			challenge: (0, __thunderid_javascript.base64urlToArrayBuffer)(publicKeyCredentialRequestOptions.challenge),
			rpId: rpIdToUse,
			...
			...publicKeyCredentialRequestOptions.allowCredentials && { allowCredentials: publicKeyCredentialRequestOptions.allowCredentials.map((cred) => ({
				...cred,
				id: (0, __thunderid_javascript.base64urlToArrayBuffer)(cred.id)
			})) }
		};
		const credential = await navigator.credentials.get({ publicKey: adjustedOptions });
		if (!credential) throw new __thunderid_javascript.ThunderIDRuntimeError("No credential returned from WebAuthn authentication", "browser-webauthn-no-credential", "browser", "The WebAuthn authentication ceremony completed but did not return a valid credential.");
		const authData = credential.response;
		const tokenResponse = {
			credential: {
				id: credential.id,
				rawId: (0, __thunderid_javascript.arrayBufferToBase64url)(credential.rawId),
				response: {
					authenticatorData: (0, __thunderid_javascript.arrayBufferToBase64url)(authData.authenticatorData),
					clientDataJSON: (0, __thunderid_javascript.arrayBufferToBase64url)(authData.clientDataJSON),
					signature: (0, __thunderid_javascript.arrayBufferToBase64url)(authData.signature),
					...authData.userHandle && { userHandle: (0, __thunderid_javascript.arrayBufferToBase64url)(authData.userHandle) }
				},
		...
		if (error instanceof __thunderid_javascript.ThunderIDRuntimeError) throw error;
		if (error instanceof Error) switch (error.name) {
			case "NotAllowedError": throw new __thunderid_javascript.ThunderIDRuntimeError("Passkey authentication was cancelled or timed out. Please try again.", "browser-webauthn-user-cancelled", "browser", "The user cancelled the WebAuthn authentication request or the request timed out.");
			case "SecurityError":
				if (error.message.includes("relying party ID") || error.message.includes("RP ID")) throw new __thunderid_javascript.ThunderIDRuntimeError("Domain mismatch error. The passkey was registered for a different domain. Please contact support or try a different authentication method.", "browser-webauthn-domain-mismatch", "browser", "The WebAuthn relying party ID does not match the current domain.");
				throw new __thunderid_javascript.ThunderIDRuntimeError("Passkey authentication failed due to a security error. Please ensure you are using HTTPS and that your browser supports passkeys.", "browser-webauthn-security-error", "browser", "A security error occurred during WebAuthn authentication.");
			case "InvalidStateError": throw new __thunderid_javascript.ThunderIDRuntimeError("No valid passkey found for this account. Please register a passkey first or use a different authentication method.", "browser-webauthn-no-passkey", "browser", "No registered passkey credentials were found for the current user account.");
			case "NotSupportedError": throw new __thunderid_javascript.ThunderIDRuntimeError("Passkey authentication is not supported on this device or browser. Please use a different authentication method.", "browser-webauthn-not-supported", "browser", "WebAuthn is not supported on the current device or browser configuration.");
			case "NetworkError": throw new __thunderid_javascript.ThunderIDRuntimeError("Network error during passkey authentication. Please check your connection and try again.", "browser-webauthn-network-error", "browser", "A network error occurred while communicating with the authenticator.");
			case "UnknownError": throw new __thunderid_javascript.ThunderIDRuntimeError("An unknown error occurred during passkey authentication. Please try again or use a different authentication method.", "browser-webauthn-unknown-error", "browser", "An unidentified error occurred during the WebAuthn authentication process.");
			default: throw new __thunderid_javascript.ThunderIDRuntimeError(`Passkey authentication failed: ${error.message}`, "browser-webauthn-general-error", "browser", `WebAuthn authentication failed with error: ${error.name}`);
		}
		throw new __thunderid_javascript.ThunderIDRuntimeError("Passkey authentication failed due to an unexpected error.", "browser-webauthn-unexpected-error", "browser", "An unexpected error occurred during WebAuthn authentication.");
	}
};
...
const resolveEmojiUrisInHtml = (html) => {
	return html.replace(/<img([^>]*)src="(emoji:[^"]+)"([^>]*)\/?>/gi, (_match, pre, src, post) => {
		const emoji = (0, __thunderid_javascript.extractEmojiFromUri)(src);
	...
	}).replace(/emoji:([^\s"<>&]+)/g, (_, rest) => (0, __thunderid_javascript.isEmojiUri)(`emoji:${rest}`) ? rest : `emoji:${rest}`);
};
var resolveEmojiUrisInHtml_default = resolveEmojiUrisInHtml;

//#endregion
//#region src/theme/themeDetection.ts
const logger = (0, __thunderid_javascript.createPackageComponentLogger)("@thunderid/browser", "ThemeDetection");
/**
...
const getActiveTheme = (mode, config = {}) => {
	...
	if (mode === "system") {
		if (typeof window !== "undefined" && window.matchMedia) return window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light";
		return __thunderid_javascript.DEFAULT_THEME;
	}
...
//#endregion
exports.ACCESS_TOKEN_INVALID = ACCESS_TOKEN_INVALID;
exports.AuthenticationHelper = AuthenticationHelper_default;
exports.BrowserHooks = Hooks;
exports.BrowserStorage = BrowserStorage_default;
exports.CHECK_SESSION_SIGNED_IN = CHECK_SESSION_SIGNED_IN;
exports.CHECK_SESSION_SIGNED_OUT = CHECK_SESSION_SIGNED_OUT;
exports.CUSTOM_GRANT_CONFIG = CUSTOM_GRANT_CONFIG;
exports.ERROR = ERROR;
exports.ERROR_DESCRIPTION = ERROR_DESCRIPTION;
exports.FetchHttpClient = FetchHttpClient_default;
exports.Hooks = Hooks_default;
exports.INITIALIZED_SILENT_SIGN_IN = INITIALIZED_SILENT_SIGN_IN;
exports.LocalStore = LocalStore_default;
exports.MemoryStore = MemoryStore_default;
exports.OP_IFRAME = OP_IFRAME;
exports.PROMPT_NONE_IFRAME = PROMPT_NONE_IFRAME;
exports.PROMPT_NONE_REQUEST_SENT = PROMPT_NONE_REQUEST_SENT;
exports.REFRESH_ACCESS_TOKEN_ERR0R = REFRESH_ACCESS_TOKEN_ERR0R;
exports.RP_IFRAME = RP_IFRAME;
exports.SET_SESSION_STATE_FROM_IFRAME = SET_SESSION_STATE_FROM_IFRAME;
exports.SILENT_SIGN_IN_STATE = SILENT_SIGN_IN_STATE;
exports.SPACryptoUtils = SPACryptoUtils_default;
exports.SPAHelper = SPAHelper_default;
exports.SPAUtils = SPAUtils_default;
exports.STATE = STATE;
exports.STATE_QUERY = STATE_QUERY;
exports.SessionStore = SessionStore_default;
exports.TOKEN_REQUEST_CONFIG_KEY = TOKEN_REQUEST_CONFIG_KEY;
exports.ThunderIDBrowserClient = ThunderIDBrowserClient_default;
exports.createClassObserver = createClassObserver;
exports.createMediaQueryListener = createMediaQueryListener;
exports.createSessionManagementHelper = SessionManagementHelper_default;
exports.detectThemeMode = detectThemeMode;
exports.getActiveTheme = getActiveTheme_default;
exports.handleWebAuthnAuthentication = handleWebAuthnAuthentication_default;
exports.hasAuthParamsInUrl = hasAuthParamsInUrl_default;
exports.hasCalledForThisInstanceInUrl = hasCalledForThisInstanceInUrl_default;
exports.http = http_default;
exports.navigate = navigate_default;
exports.resolveEmojiUrisInHtml = resolveEmojiUrisInHtml_default;
