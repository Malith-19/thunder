...
function isMissingOnboardingFlow(error) {
	...
	return apiError?.code === FLOW_NOT_FOUND_ERROR_CODE || flowError.code === FLOW_NOT_FOUND_ERROR_CODE || containsFlowNotFoundText(apiError?.message) || containsFlowNotFoundText(apiError?.description) || containsFlowNotFoundText(flowError.message) || containsFlowNotFoundText(flowError.failureReason);
}
...
function InviteUserStepContent({ renderProps, flowError, handleClose, onResetLocalState }) {
	...
	if (isLoading && !components?.length) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(__wso2_oxygen_ui.Box, {
		sx: {
			display: "flex",
	...
	if (!components?.length) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(__wso2_oxygen_ui.Box, {
		sx: {
			display: "flex",
			justifyContent: "center",
			p: 4
