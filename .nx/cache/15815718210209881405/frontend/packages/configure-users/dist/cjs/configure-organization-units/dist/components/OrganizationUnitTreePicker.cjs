...
//#region ../configure-organization-units/dist/components/OrganizationUnitTreePicker.js
...
function PickerTreeItem(allProps) {
	...
	const isLoadMoreItem = itemId.endsWith(require_organization_unit_tree_constants.organization_unit_tree_constants_default.LOAD_MORE_SUFFIX);
	const isEmptyPlaceholder = itemId.endsWith(require_organization_unit_tree_constants.organization_unit_tree_constants_default.EMPTY_SUFFIX);
	const isLoadingPlaceholder = !isEmptyPlaceholder && !isLoadMoreItem && (itemData?.isPlaceholder ?? itemId.endsWith(require_organization_unit_tree_constants.organization_unit_tree_constants_default.PLACEHOLDER_SUFFIX));
	const isItemLoading = loadingItemsProp?.has(itemId);
	if (isLoadMoreItem) {
		const parentId = itemId.replace(require_organization_unit_tree_constants.organization_unit_tree_constants_default.LOAD_MORE_SUFFIX, "");
...
function OrganizationUnitTreePicker({ id = void 0, value, onChange, error = false, helperText = "", rootOuId = void 0, maxHeight = 300 }) {
	...
	const { data, isLoading } = require_useGetOrganizationUnits.useGetOrganizationUnits(void 0, !rootOuId);
	const { data: rootOuData, isLoading: isRootOuLoading, error: rootOuError } = require_useGetOrganizationUnit.useGetOrganizationUnit(rootOuId);
	const { data: rootOuChildrenData, isLoading: isRootOuChildrenLoading, error: rootOuChildrenError } = require_useGetChildOrganizationUnits.useGetChildOrganizationUnits(rootOuId);
	...
	const itemMap = (0, react.useMemo)(() => require_buildItemMap.buildItemMap(treeItems), [treeItems]);
	(0, react.useEffect)(() => {
		setTreeItems([]);
		setExpandedItems([]);
	...
	(0, react.useEffect)(() => {
		...
		if (data?.organizationUnits && data.organizationUnits.length > 0 && treeItems.length === 0) {
			const items = require_buildTreeItems.buildTreeItems(data.organizationUnits);
			if (data.organizationUnits.length < data.totalResults) items.push({
				id: require_organization_unit_tree_constants.organization_unit_tree_constants_default.ROOT_LOAD_MORE_ID,
				label: "",
				handle: "",
				isPlaceholder: true
			});
	...
	(0, react.useEffect)(() => {
		...
		const childItems = require_buildTreeItems.buildTreeItems(rootOuChildrenData.organizationUnits);
		if (rootOuChildrenData.organizationUnits.length < rootOuChildrenData.totalResults) childItems.push({
			id: `${rootOuId}${require_organization_unit_tree_constants.organization_unit_tree_constants_default.LOAD_MORE_SUFFIX}`,
			label: "",
			handle: "",
			isPlaceholder: true
		});
		const rootChildren = rootOuChildrenData.organizationUnits.length > 0 ? childItems : [{
			id: `${rootOuId}${require_organization_unit_tree_constants.organization_unit_tree_constants_default.EMPTY_SUFFIX}`,
	...
	const fetchChildPage = (0, react.useCallback)(async (parentId, offset) => queryClient.fetchQuery({
		queryKey: [
			require_organization_unit_query_keys.organization_unit_query_keys_default.CHILD_ORGANIZATION_UNITS,
			parentId,
			{
				limit: require_organization_unit_tree_constants.organization_unit_tree_constants_default.PAGE_SIZE,
				offset
			}
		],
		queryFn: async () => require_fetchChildOrganizationUnits.fetchChildOrganizationUnits(http, getServerUrl(), parentId, {
			limit: require_organization_unit_tree_constants.organization_unit_tree_constants_default.PAGE_SIZE,
			offset
		}),
		staleTime: 0
	...
	const buildChildItems = (0, react.useCallback)((parentId, result, offset) => {
		...
		if (childOUs.length === 0 && offset === 0) return [{
			id: `${parentId}${require_organization_unit_tree_constants.organization_unit_tree_constants_default.EMPTY_SUFFIX}`,
			label: t("organizationUnits:listing.treeView.noChildren"),
			handle: "",
			isPlaceholder: true
		}];
		const items = require_buildTreeItems.buildTreeItems(childOUs);
		if (offset + childOUs.length < result.totalResults) items.push({
			id: `${parentId}${require_organization_unit_tree_constants.organization_unit_tree_constants_default.LOAD_MORE_SUFFIX}`,
			label: "",
			handle: "",
			isPlaceholder: true
		});
	...
	const fetchChildOUs = (0, react.useCallback)(async (parentId) => {
		...
		try {
			...
			setTreeItems((prev) => require_updateTreeItemChildren.updateTreeItemChildren(prev, parentId, childItems));
	...
	const handleRootLoadMore = (0, react.useCallback)(async () => {
		...
		try {
			...
			const newItems = require_buildTreeItems.buildTreeItems(result.organizationUnits);
			...
			setTreeItems((prev) => {
				return [...prev.filter((item) => item.id !== require_organization_unit_tree_constants.organization_unit_tree_constants_default.ROOT_LOAD_MORE_ID), ...newItems];
			});
		} catch (_error) {
	...
	const handleLoadMore = (0, react.useCallback)(async (parentId) => {
		if (parentId === require_organization_unit_tree_constants.organization_unit_tree_constants_default.ROOT_PARENT_ID) {
			await handleRootLoadMore();
			return;
		}
		setLoadMoreLoadingItems((prev) => new Set(prev).add(parentId));
		try {
			const offset = childOffsets.get(parentId) ?? require_organization_unit_tree_constants.organization_unit_tree_constants_default.PAGE_SIZE;
			...
			setTreeItems((prev) => require_appendTreeItemChildren.appendTreeItemChildren(prev, parentId, newItems));
		} catch (_error) {
	...
	const combinedLoadMoreLoadingItems = (0, react.useMemo)(() => {
		...
		combined.add(require_organization_unit_tree_constants.organization_unit_tree_constants_default.ROOT_PARENT_ID);
		return combined;
	...
	const handleSelectedItemsChange = (0, react.useCallback)((_event, itemId) => {
		if (itemId && !itemId.endsWith(require_organization_unit_tree_constants.organization_unit_tree_constants_default.PLACEHOLDER_SUFFIX) && !itemId.endsWith(require_organization_unit_tree_constants.organization_unit_tree_constants_default.EMPTY_SUFFIX) && !itemId.endsWith(require_organization_unit_tree_constants.organization_unit_tree_constants_default.LOAD_MORE_SUFFIX)) onChange(itemId);
	}, [onChange]);
	...
	if (isTreeLoading) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(__wso2_oxygen_ui.Box, {
		sx: {
			display: "flex",
			justifyContent: "center",
			py: 4
...
//#endregion
exports.OrganizationUnitTreePicker = OrganizationUnitTreePicker;