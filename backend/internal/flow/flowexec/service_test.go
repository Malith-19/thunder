...
func TestExecute_EngineError_InvalidChallengeToken_PreservesContext(t *testing.T) {
	...
	mockEngine.EXPECT().Execute(mock.Anything).Return(
		FlowStep{Status: common.FlowStatusIncomplete, Error: &challengeTokenErr}, nil)
...
func TestExecute_EngineError_NewFlow_ContextNeverRemoved(t *testing.T) {
	...
	mockEntitySvc.EXPECT().AuthenticateEntityByID(mock.Anything, "test-app",
		map[string]interface{}{fieldAppSecret: "valid-secret"}).
...
func (s *ServiceTestSuite) TestExecute_NewFlow_IncompleteStoresContext() {
	...
	mockEntitySvc := entitymock.NewEntityServiceInterfaceMock(s.T())
	mockInboundClient.EXPECT().GetOAuthProfileByEntityID(mock.Anything, appID).Return(nil, nil)
	mockEntitySvc.EXPECT().AuthenticateEntityByID(mock.Anything, appID,
		map[string]interface{}{fieldAppSecret: "valid-secret"}).
		Return(&entity.AuthenticateResult{EntityID: appID}, nil)
	mockInboundClient.EXPECT().GetInboundClientByEntityID(mock.Anything, appID).Return(
...
// A public redirect client (browser SPA) is blocked because its profile uses authorization_code —
// the block is driven by the redirect grant, not by the publicClient flag.
func (s *ServiceTestSuite) TestExecute_NewFlow_PublicRedirectApp_Blocked() {
	t := s.T()
	testConfig := &config.Config{}
	config.ResetServerRuntime()
	_ = config.InitializeServerRuntime("/tmp/test", testConfig)
	...
	mockObservability := observabilitymock.NewObservabilityServiceInterfaceMock(t)
	mockActorProvider.EXPECT().GetOAuthProfileByID(mock.Anything, "test-app").Return(
		&inboundmodel.OAuthProfile{GrantTypes: []string{"authorization_code"}, PublicClient: true}, nil)
	mockObservability.EXPECT().IsEnabled().Return(false)
...
func (s *ServiceTestSuite) TestExecute_NewFlow_BackendApp_InvalidSecret_Rejected() {
	...
	mockActorProvider.EXPECT().GetOAuthProfileByID(mock.Anything, "test-app").Return(
		&inboundmodel.OAuthProfile{GrantTypes: []string{"client_credentials"}, PublicClient: false}, nil)
	mockEntitySvc.EXPECT().AuthenticateEntityByID(mock.Anything, "test-app",
		map[string]interface{}{fieldAppSecret: "wrong-secret"}).
...
// An embedded server-side app has no OAuth profile. It must present a valid App Secret, and when
...
// An embedded server-side app (no OAuth profile) that omits the App Secret is rejected.
...
// A missing OAuth profile is the embedded server-side app case: no profile means the App Secret
// is required, so an empty secret is rejected rather than allowed through.
func (s *ServiceTestSuite) TestCheckDirectFlowInitiationAllowed_NoProfileRequiresAppSecret() {
	t := s.T()
	mockActorProvider := actorprovidermock.NewActorProviderInterfaceMock(t)
	...
	mockActorProvider.EXPECT().GetInboundClientByID(mock.Anything, "embedded-app").Return(
		&inboundmodel.InboundClient{ID: "embedded-app"}, nil)
...
// An application that has neither an OAuth profile nor an inbound-client row does not exist; the
// guard must surface that as an invalid-app-ID error rather than demanding an App Secret.
func (s *ServiceTestSuite) TestCheckDirectFlowInitiationAllowed_UnknownAppInvalidAppID() {
	...
	mockActorProvider.EXPECT().GetInboundClientByID(mock.Anything, "app-notfound").Return(
		(*inboundmodel.InboundClient)(nil), &actorprovider.ErrorActorNotFound)
