...
// are not eligible. Mobile apps authenticate via client attestation and m2m apps obtain tokens
// directly, so neither is issued a Flow Secret regardless of their OAuth config shape.
func isFlowSecretEligible(appType model.ApplicationType,
	inboundAuthConfig *providers.InboundAuthConfigWithSecret) bool {
	if appType == model.ApplicationTypeMobile || appType == model.ApplicationTypeM2M {
		return false
	}
	if inboundAuthConfig == nil || inboundAuthConfig.OAuthConfig == nil {
		return true
